...
models = [(BiAA, False), (SBM, True), (SBM, False), (DBiAA, False), (DSBM, True), (DSBM, False)]
...
for sim_i in tqdm(range(n_sims), leave=False):
    ...
    for k in tqdm(range(k_min, k_max), leave=False):
        ...
        biaa_model = None
        for assignment, (model, init) in tqdm(list(product(assignments, models)), leave=False):
            auc_j = -np.inf
            prauc_j = -np.inf
            ...
            data = {"loss": best_model.losses[::20], "step": np.arange(len(best_model.losses))[::20],
                    ...
                    "assignment": assignment, "k": k, "iteration": sim_i, "init": init}
            results_loss_i = pd.DataFrame(data=data)
            results_loss = pd.concat([results_loss, results_loss_i], ignore_index=True)

    results.to_csv(f"results/{dataset}_results_auc_init.csv")
    results_loss.to_csv(f"results/{dataset}_results_loss_init.csv")
