    ...
    def __init__(
        ...
        if aa_model is not None:
            ...
            if degree_correction:
                self._DC_params = [
                    torch.nn.Parameter(dc.detach().to(device), requires_grad=True)
                    for dc in aa_model._DC_params
                ]
                self._DC = map_params(self._DC_params, relations)

            self._A_params = [
                torch.nn.Parameter(a.detach().to(device), requires_grad=True)
                for a in aa_model._A_params
            ]

            self._A = map_params(self._A_params, relations)

            self._Z = torch.nn.Parameter(torch.logit(aa_model.Z.detach()).to(device), requires_grad=True)
        else:
            self._DC = None
            if degree_correction:
                self._DC_params = [
            ...
            # archetypes
            z_shape = tuple(self.k) + tuple(self.s[self.n:])
            self._Z = torch.nn.Parameter(torch.randn(z_shape, device=self.device), requires_grad=True)
