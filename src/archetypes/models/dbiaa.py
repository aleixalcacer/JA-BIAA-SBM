...
class DBiAA(nn.Module):
    def __init__(self, k, data, likelihood="normal", assignment="soft"):
        ...
        self._a = torch.nn.Parameter(torch.sigmoid(torch.rand(self.m)), requires_grad=True)
        self._d = torch.nn.Parameter(torch.sigmoid(torch.rand(self.n)), requires_grad=True)
        ...
        if assignment == "soft":
            ...
            self._A = hardmax(torch.rand(self.m, self.k[0]), dim=1)
            self._D = hardmax(torch.rand(self.k[1], self.n), dim=0)
    ...
    def fit(self, iterations, learning_rate=0.01, print_loss=False):
        if self._assignment == "soft":
            optimizer_out = torch.optim.Adam(params=[self._a, self._A, self._D, self._d], lr=learning_rate)
            optimizer_in = torch.optim.Adam(params=[self._B, self._C], lr=learning_rate)
        else:
            optimizer_out = None
            optimizer_in = torch.optim.Adam(params=[self._a, self._B, self._C, self._d], lr=learning_rate)
